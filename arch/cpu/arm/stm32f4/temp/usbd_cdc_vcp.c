...
// Function pointers
static void (*vcp_input_handler)(unsigned char c) = 0;
static void (*vcp_data_available_handler)(void) = 0;
...
void VCP_set_input_handler(void (*input_handler)(unsigned char c)) {
	vcp_input_handler = input_handler;
}

void VCP_set_data_available_handler(void (*data_available_handler)(void)) {
	vcp_data_available_handler = data_available_handler;
}
...
static uint16_t VCP_DataRx(uint8_t* Buf, uint32_t Len) {
	...
	if(vcp_data_available_handler != 0 && vcp_input_handler == 0) {
		vcp_data_available_handler();
	}
